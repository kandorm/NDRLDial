...
class DiaActItem(object):
    ...
    :type slot: str or unicode
    :param op: comparative operation, e.g. '=' or '!='
    :type op: str or unicode
    :param val: value name
    :type val: str or unicode
    """
    ...
    def match(self, other):
        ...
        if self.val is None or other.val is None:
            print 'None value is given in comparison between %s and %s' % str(self), str(other)
...
class DiaAct(object):
    ...
    self.dact = ``{'act': acttype,'items': [(slot1, op1, value1), ..])}``
